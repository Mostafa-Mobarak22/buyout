...
export default function AllProperty() {   
    const confirm = (id ,name,email)=>{
        ...
        if (result.isConfirmed) {
            Swal.fire({
                ...
                icon: "success",
                confirmButton: deleteProperty(id ,name,email)
            });
        }
      });
    }  
    ...
    async function getUsers() {
      ...
      async function confirmProperty(name,email){
            const {data} = await axios.post("http://127.0.0.1:8000/user/confirm/",{
                name:name,
                email:email,
                massage:"Your property Is Deleted"
            })
            console.log(data);
    }
  return <>
    ...
    return    <tr key={key}>
        <td className="px-4 py-4 text-sm text-gray-950 whitespace-nowrap">{value.id}</td>
            ...
            <td className="px-12 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">
                {
                    !value.is_published ?    <div className="inline-flex items-center px-3 py-1 rounded-full gap-x-2 bg-emerald-100/60 dark:bg-gray-800">
                    <span className="h-1.5 w-1.5 rounded-full bg-red-700"></span>

                    <h2 className="text-sm font-normal text-red-700">Not Published</h2>
                </div> :
                    <div className="inline-flex items-center px-3 py-1 rounded-full gap-x-2 bg-emerald-100/60 dark:bg-gray-800">
                        <span className="h-1.5 w-1.5 rounded-full bg-emerald-500"></span>

                        <h2 className="text-sm font-normal text-emerald-500">Published</h2>
                </div>
                }
            ...
            <td className="px-4 py-4 text-sm whitespace-nowrap">
                <div className="flex items-center gap-x-6">
                    <button onClick={()=>{confirm(value.id,value.user_id.user_name,value.user_id.email)}} className="text-gray-950 transition-colors duration-200 dark:hover:text-red-500 hover:text-red-500 focus:outline-none">
            ...
            <td className="px-4 py-4 text-sm whitespace-nowrap">
                <div className="flex items-center gap-x-6">
                    ...
                    <Link to={"/admin/editproperty/"+value.id} className="text-gray-950 transition-colors duration-200 dark:hover:text-yellow-500 hover:text-yellow-500 focus:outline-none">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-5 h-5">
